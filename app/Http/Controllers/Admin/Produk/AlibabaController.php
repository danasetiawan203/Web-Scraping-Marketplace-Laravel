...
class AlibabaController extends Controller
{
  public function index() {
    $alibabas=Alibaba::where('id_scraping', \Auth::user()->id)->get();
    $cats1 = Categori1::all();
    return view('admin.produk.alibaba',['alibabas' => $alibabas, 'cats1' => $cats1]);
}
          //data kategori1
    ...
    public function cat3($id) {
          ...
          //menghapus data alibaba
    public function hapusmodalalibaba(Request $req) {
            Alibaba::find($req->id)->delete();
            return redirect()->back()->with('error', 'Anda Telah Menghapus Produk Alibaba.');
          }

          //memanggil data dari produk alibaba
    public function editmodalalibaba($id) {

            $alibabas_edit = Alibaba::where('id','=', $id)->get();
            return response()->json($alibabas_edit);

          }

          //untuk edit produk alibaba
    public function updatemodalalibaba(Request $req) {
      $this->validate($req, [        
        ...
        // 'deskripsi.required' => 'Deskripsi harus diisi tidak boleh kosong',
            ...
            $alibabas_update = Alibaba::where('id', $req->id_edit)
            ->update([
                'url' => $req->modal_url,
            ...
            return redirect()->back()->with('success', 'Data Alibaba Telah Diperbarui.');
        }
}
