...
class TokopediaController extends Controller
{
  public function index() {
    $tokopedias=Tokopedia::where('id_scraping', \Auth::user()->id)->get();
    $cats1 = Categori1::all();
    return view('admin.produk.tokopedia',['tokopedias' => $tokopedias, 'cats1' => $cats1]);
 }
...
public function cat3($id) {
  ...
  public function hapusmodaltokopedia(Request $req) {
          Tokopedia::find($req->id)->delete();
          return redirect()->back()->with('error', 'Anda Telah Menghapus Produk Tokopedia.');
  }


  public function editmodaltokopedia($id) {

    // $tokopedias = Tokopedia::where('id', $id)->get();
          $tokopedias_edit = Tokopedia::where('id','=', $id)->get();
          return response()->json($tokopedias_edit);

  }

  public function updatemodaltokopedia(Request $req) {
    ...
    $this->validate($req, [        
      ...
      // 'deskripsi.required' => 'Deskripsi harus diisi tidak boleh kosong',
          ...
          $tokopedias_update = Tokopedia::where('id', $req->id_edit)
          ->update([
              'url' => $req->modal_url,
