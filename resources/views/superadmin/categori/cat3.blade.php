...
@section('content')
    										...
    										<ul class="icons-list">
    											<li class="dropdown">
    												...
    												<ul class="dropdown-menu dropdown-menu-right">
															...
															<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$cat3->id}}"><i class="fas fa-edit"></i> Edit</a></li>
															<li><a href="#modalHapus" class="hapusbutton" data-toggle="modal" data-id="{{$cat3->id}}" data-nama="{{$cat3->nama}}"><i class="fas fa-trash-alt"></i> Hapus</a></li>
    												</ul>
    											</li>
    										</ul>
    									</td>
	...
	<div id="modal_tambah" class="modal fade" >
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<form action="{{route('tambah.cat3')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
					<div class="modal-body">
						<div class="form-group">
							<label class="control-label col-sm-3">ID</label>
							<div class="col-sm-9">
								<input type="number" name="id" id="id" value="{{$max}}" readonly class="form-control">
							</div>
						</div>

						<div class="form-group">
							<label class="control-label col-sm-3">ID Categori2</label>
							<div class="col-sm-9">
								<select class="select-search select-border-color border-warning" name="id_cat2" id="id_cat2">
									<option selected="true" disabled>Pilih Categori</option>
									@foreach ($categori2 as $cat2)
										<option value="{{$cat2->id}}">{{$cat2->nama}}</option>
									@endforeach
								</select>
							</div>
						</div>
	...
	<div id="modal_edit" class="modal fade" >
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<form action="{{route('update.cat3')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
					...
					<div class="modal-body">
						<div class="form-group">
							<label class="control-label col-sm-3">ID</label>
							<div class="col-sm-9">
								<input type="number" name="modal_id" id="modal_id" value="{{$max}}" readonly  class="form-control">
							</div>
						</div>
						...
						<div class="form-group">
							<label class="control-label col-sm-3">ID Categori2</label>
							<div class="col-sm-9">
								<select class="select-search select-border-color border-warning" name="modal_cat2" id="modal_cat2">
									<option selected="true" disabled>Pilih Categori</option>
									@foreach ($categori2 as $cat2)
										<option value="{{$cat2->id}}">{{$cat2->nama}}</option>
									@endforeach
	...
	<div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form action="{{route('hapus.cat3')}}" method="post">
						@csrf
	...
	<script>
		...
		// $('.editbutton').click(function() {
			...
			$.get('{{URL::to("superadmin/categori3/edit") }}/' + iD, function(data){
				$.each (data, function (index, edit_cat3){
				$('#id_edit').val(iD);
				$('#modal_id').val(edit_cat3.id);
				$("#modal_nama").val(edit_cat3.nama);
				$("#modal_cat2").val(edit_cat3.id_cat2).change();
				});
			});
