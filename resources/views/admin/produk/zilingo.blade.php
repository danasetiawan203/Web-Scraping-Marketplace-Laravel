...
@section('content')
				...
				<div class="page-header">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Sistem Informasi Webscraping</span> - Data Produk Zilingo</h4>
						</div>
					...
					<div class="breadcrumb-line breadcrumb-line-component">
						<ul class="breadcrumb">
							...
							<li class="active">Data Produk Zilingo</li>
						</ul>
					</div>
				</div>
				<!-- /page header -->
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Data Produk Zilingo</h5>
						...
						<table class="table datatable-basic">
							...
							<tbody>
								@foreach ($zilingos as $zilingo)
						<tr>

							<td>
								<img src="{{$zilingo->foto}}" alt="" class="img-rounded img-preview">
							</td>
							<td>{{$zilingo->nama_barang}}</td>
							<td>{{$zilingo->harga}}</td>
							<td>{{$zilingo->stock}}</td>
							<td>
								@if ($zilingo->stock>=1)
										...
										<ul class="icons-list">
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li><a href="{{$zilingo->url}}"><i class="fas fa-globe"></i> GO TO URL</a></li>
													<li><a href="{{route('lihatproduk', ['mp' => 'zilingo','id'=>$zilingo->id])}}"><i class="fas fa-eye"></i> Lihat</a></li>
													<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$zilingo->id}}"><i class="fas fa-edit"></i> Edit</a></li>
													<li><a href="#modalHapus" class="hapusbutton" data-toggle="modal" data-id="{{$zilingo->id}}" data-nama="{{$zilingo->nama_barang}}"><i class="fas fa-trash-alt"></i> Hapus</a></li>
												</ul>
											</li>
										</ul>
									</td>
	...
	<div id="modal_edit" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-lg" style="width:1250px">
			<div class="modal-content" style="width:1200px">
				...
				<form action="{{route('updatemodalzilingo')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
	...
	<div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form action="{{route('hapusmodalzilingo')}}" method="post">
						@csrf
	...
	<script>
		...
		$(document).on('click', '.editbutton', function(e){
			// $('.editbutton').click(function(){
				...
				$.get('{{URL::to("dropshipper/zilingo/edit") }}/' + iD, function (data){
					$.each(data, function (index,zilingos_edit) {
						$('#id_edit').val(iD);
						$('#modal_nama_barang').val(zilingos_edit.nama_barang);
						$("#modal_url2").val(zilingos_edit.url);
						$("#modal_foto").attr('src', zilingos_edit.foto);
						$("#modal_foto2").val(zilingos_edit.foto);
						$('#modal_harga').val(zilingos_edit.harga);
						// $('#modal_deskripsi').data("wysihtml5").editor.setValue(tokopedias_edit.deskripsi);
						CKEDITOR.instances.editor1.setData(zilingos_edit.deskripsi);
						$('#modal_berat').val(zilingos_edit.berat);
						$('#modal_stock').val(zilingos_edit.stock);
						$('#modal_categori1').val(zilingos_edit.cat1).change();
						$('#modal_categori2').val(zilingos_edit.cat2).change();
						$('#modal_categori3').val(zilingos_edit.cat3);
						$('#modal_ukuran').val(zilingos_edit.ukuran).change();
						$('#modal_warna').val(zilingos_edit.warna);
						var id_cate2 = zilingos_edit.cat2;
						var id_cate3 = zilingos_edit.cat3;
						// console.log(id_cat2);
						$.get('{{ URL::to("ajaxcat2/") }}/' + zilingos_edit.cat1, function(data){
							$('#modal_categori2').empty();
							$.each(data, function(index, id_cat2){
								var select="";
						...
						$.get('{{ URL::to("ajaxcat3/") }}/' + zilingos_edit.cat2, function(data){
	...
	if($('#succesToast').is(':visible')) {
		toastr.success("Data Zilingo","Berhasil");
	}
		</script> --}}
