...
@section('content')
										...
										<ul class="icons-list">
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													...
													<li><a href="{{route('lihatproduk', ['mp' => 'lazada','id'=>$lazada->id])}}"><i class="fas fa-eye"></i> Lihat</a></li>
													<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$lazada->id}}"><i class="fas fa-edit"></i> Edit</a></li>
	...
	<div id="modal_edit" class="modal fade" >
		<div class="modal-dialog modal-lg" style="width:1250px">
			<div class="modal-content" style="width:1200px">
				...
				<form action="{{route('updatemodallazada')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
	...
	<div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form action="{{route('hapusmodallazada')}}" method="post">
						@csrf
	...
	$('#modal_categori2').on('change', function(e){
		...
		$(document).on('click', '.editbutton', function(e){
			// $('.editbutton').click(function(){
				...
				$.get('{{URL::to("dropshipper/lazada/edit") }}/' + iD, function (data){
					$.each(data, function (index,lazadas_edit) {
						$('#id_edit').val(iD);
						$('#modal_nama_barang').val(lazadas_edit.nama_barang);
						$("#modal_url2").val(lazadas_edit.url);
						$("#modal_foto").attr('src', lazadas_edit.foto);
						$("#modal_foto2").val(lazadas_edit.foto);
						$('#modal_harga').val(lazadas_edit.harga);
						// $('#modal_deskripsi').data("wysihtml5").editor.setValue(tokopedias_edit.deskripsi);
						CKEDITOR.instances.editor1.setData(lazadas_edit.deskripsi);
						$('#modal_berat').val(lazadas_edit.berat);
						$('#modal_stock').val(lazadas_edit.stock);
						$('#modal_categori1').val(lazadas_edit.cat1).change();
						$('#modal_categori2').val(lazadas_edit.cat2).change();
						$('#modal_categori3').val(lazadas_edit.cat3);
						$('#modal_ukuran').val(lazadas_edit.ukuran).change();
						$('#modal_warna').val(lazadas_edit.warna);
						var id_cate2 = lazadas_edit.cat2;
						var id_cate3 = lazadas_edit.cat3;
						// console.log(id_cat2);
						$.get('{{ URL::to("ajaxcat2/") }}/' + lazadas_edit.cat1, function(data){
							$('#modal_categori2').empty();
							$.each(data, function(index, id_cat2){
								var select="";
						...
						$.get('{{ URL::to("ajaxcat3/") }}/' + lazadas_edit.cat2, function(data){
	...
	if($('#succesToast').is(':visible')) {
		toastr.success("Data Lazada","Berhasil");
	}
		</script> --}}
