...
@section('content')
				...
				<div class="page-header">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Sistem Informasi Webscraping</span> - Data Produk Bukalapak</h4>
						</div>
					...
					<div class="breadcrumb-line breadcrumb-line-component">
						<ul class="breadcrumb">
							...
							<li class="active">Data Produk Bukalapak</li>
						</ul>
					</div>
				</div>
				<!-- /page header -->
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Data Produk Bukalapak</h5>
						...
						<table class="table datatable-basic">
							...
							<tbody>
								@foreach ($bukalapaks as $bukalapak )
								<tr>

										<td>
												<img src="{{$bukalapak->foto}}" alt="" class="img-rounded img-preview">
										</td>
										<td>{{$bukalapak->nama_barang}}</td>
										<td>{{$bukalapak->harga}} </td>
										<td>{{$bukalapak->stock}}</td>
										<td>
											@if ($bukalapak->stock>=1)
											...
											<ul class="icons-list">
												<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-menu-right">
														<li><a href="{{$bukalapak->url}}"><i class="fas fa-globe"></i> GO TO URL</a></li>
														<li><a href="{{route('lihatproduk', ['mp' => 'bukalapak','id'=>$bukalapak->id])}}"><i class="fas fa-eye"></i> Lihat</a></li>
														<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$bukalapak->id}}"><i class="fas fa-edit"></i> Edit</a></li>
														<li><a href="#modalHapus" class="hapusbutton" data-toggle="modal" data-id="{{$bukalapak->id}}" data-nama="{{$bukalapak->nama_barang}}"><i class="fas fa-trash-alt"></i> Hapus</a></li>
													</ul>
												</li>
											</ul>
										</td>
	...
	<div id="modal_edit" class="modal fade" >
		<div class="modal-dialog modal-lg" style="width:1250px">
			<div class="modal-content" style="width:1200px">
				...
				<form action="{{route('updatemodalbukalapak')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
	...
	<div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form action="{{route('hapusmodalbukalapak')}}" method="post">
						@csrf
	...
	// $('.editbutton').click(function(){
		...
		$.get('{{URL::to("dropshipper/bukalapak/edit") }}/' + iD, function (data){
			$.each(data, function (index,bukalapaks_edit) {
				$('#id_edit').val(iD);
				$('#modal_nama_barang').val(bukalapaks_edit.nama_barang);
				$("#modal_url2").val(bukalapaks_edit.url);
				$("#modal_foto").attr('src', bukalapaks_edit.foto);
				$("#modal_foto2").val(bukalapaks_edit.foto);
				$('#modal_harga').val(bukalapaks_edit.harga);
				// $('#modal_deskripsi').data("wysihtml5").editor.setValue(bukalapaks_edit.deskripsi);
				CKEDITOR.instances.editor1.setData(bukalapaks_edit.deskripsi);
				$('#modal_berat').val(bukalapaks_edit.berat);
				$('#modal_stock').val(bukalapaks_edit.stock);
				$('#modal_categori1').val(bukalapaks_edit.cat1).change();
				// $('#modal_categori2').val(bukalapaks_edit.cat2).change();
				// $('#modal_categori3').val(bukalapaks_edit.cat3);
				$('#modal_ukuran').val(bukalapaks_edit.ukuran).change();
				$('#modal_warna').val(bukalapaks_edit.warna);
				var id_cate2 = bukalapaks_edit.cat2;
				var id_cate3 = bukalapaks_edit.cat3;
						// console.log(id_cat2);
						$.get('{{ URL::to("ajaxcat2/") }}/' + bukalapaks_edit.cat1, function(data){
							$('#modal_categori2').empty();
							$.each(data, function(index, id_cat2){
								var select="";
					...
					$.get('{{ URL::to("ajaxcat3/") }}/' + bukalapaks_edit.cat2, function(data){
...
if($('#succesToast').is(':visible')) {
	toastr.success("Data Bukalapak","Berhasil");
}
	</script> --}}
