...
@section('content')
										...
										<ul class="icons-list">
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													...
													<li><a href="{{route('lihatproduk', ['mp' => 'zalora','id'=>$zalora->id])}}"><i class="fas fa-eye"></i> Lihat</a></li>
													<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$zalora->id}}"><i class="fas fa-edit"></i> Edit</a></li>
	...
	<div id="modal_edit" class="modal fade" >
		<div class="modal-dialog modal-lg" style="width:1250px">
			<div class="modal-content" style="width:1200px">
				...
				<form action="{{route('updatemodalzalora')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
	...
	<div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form action="{{route('hapusmodalzalora')}}" method="post">
						@csrf
	...
	<script>
		...
		$(document).on('click', '.editbutton', function(e){
			// $('.editbutton').click(function(){
				...
				$.get('{{URL::to("dropshipper/zalora/edit") }}/' + iD, function (data){
					$.each(data, function (index,zaloras_edit) {
						$('#id_edit').val(iD);
						$('#modal_nama_barang').val(zaloras_edit.nama_barang);
						$("#modal_url2").val(zaloras_edit.url);
						$("#modal_foto").attr('src', zaloras_edit.foto);
						$("#modal_foto2").val(zaloras_edit.foto);
						$('#modal_harga').val(zaloras_edit.harga);
						// $('#modal_deskripsi').data("wysihtml5").editor.setValue(tokopedias_edit.deskripsi);
						CKEDITOR.instances.editor1.setData(zaloras_edit.deskripsi);
						$('#modal_berat').val(zaloras_edit.berat);
						$('#modal_stock').val(zaloras_edit.stock);
						$('#modal_categori1').val(zaloras_edit.cat1).change();
						// $('#modal_categori2').val(zaloras_edit.cat2).change();
						// $('#modal_categori3').val(zaloras_edit.cat3);
						$('#modal_ukuran').val(zaloras_edit.ukuran).change();
						$('#modal_warna').val(zaloras_edit.warna);
						var id_cate2 = zaloras_edit.cat2;
						var id_cate3 = zaloras_edit.cat3;
						// console.log(id_cat2);
						$.get('{{ URL::to("ajaxcat2/") }}/' + zaloras_edit.cat1, function(data){
							$('#modal_categori2').empty();
							$.each(data, function(index, id_cat2){
								var select="";
						...
						$.get('{{ URL::to("ajaxcat3/") }}/' + zaloras_edit.cat2, function(data){
	...
	if($('#succesToast').is(':visible')) {
		toastr.success("Data Zalora","Berhasil");
	}
		</script> --}}
