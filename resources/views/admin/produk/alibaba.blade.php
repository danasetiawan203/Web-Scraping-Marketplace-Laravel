				...
				<div class="page-header">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Sistem Informasi Webscraping</span> - Data Produk Alibaba</h4>
						</div>
					...
					<div class="breadcrumb-line breadcrumb-line-component">
						<ul class="breadcrumb">
							...
							<li class="active">Data Produk Alibaba</li>
						</ul>
					</div>
				</div>
				<!-- /page header -->
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Data Produk Alibaba</h5>
						...
						<table class="table datatable-basic">
							...
							<tbody>
								{{-- untuk looping data produk alibaba--}}
								@foreach ($alibabas as $alibaba)
								<tr>
									<td>
										<img src="{{$alibaba->foto}}" alt="" class="img-rounded img-preview">
									</td>
									<td>{{$alibaba->nama_barang}}</td>
									<td>{{$alibaba->harga}}</td>
									<td>{{$alibaba->stock}}</td>
									<td>
										@if ($alibaba->stock>=1)
										...
										<ul class="icons-list">
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li><a href="{{$alibaba->url}}"><i class="fas fa-globe"></i> GO TO URL</a></li>
													<li><a href="{{route('lihatproduk', ['mp' => 'alibaba','id'=>$alibaba->id])}}"><i class="fas fa-eye"></i> Lihat</a></li>
													<li><a href="#modal_edit" class="editbutton" data-toggle="modal" data-id="{{$alibaba->id}}"><i class="fas fa-edit"></i> Edit</a></li>
													<li><a href="#modalHapus" class="hapusbutton" data-toggle="modal" data-id="{{$alibaba->id}}" data-nama="{{$alibaba->nama_barang}}"><i class="fas fa-trash-alt"></i> Hapus</a></li>
												</ul>
											</li>
										</ul>
									</td>
	...
	<div id="modal_edit" class="modal fade">
		<div class="modal-dialog modal-lg" style="width:1250px">
			<div class="modal-content" style="width:1200px">
				...
				<form action="{{route('updatemodalalibaba')}}" class="form-horizontal" method="post">
					{{ csrf_field() }}
										...
										<div class="col-lg-9">
											<select class="select-search select-border-color border-warning" name="modal_ukuran" id="modal_ukuran">
												<optgroup label="Marketplace">
													<option selected="true" disabled>Ukuran</option>
...
<script>
	...
	$(document).on('click', '.editbutton', function(e){
		// $('.editbutton').click(function(){
			...
			$.get('{{URL::to("dropshipper/alibaba/edit") }}/' + iD, function (data){
				$.each(data, function (index,alibabas_edit) {
					$('#id_edit').val(iD);
					$('#modal_nama_barang').val(alibabas_edit.nama_barang);
					$("#modal_url2").val(alibabas_edit.url);
					$("#modal_foto").attr('src', alibabas_edit.foto);
					$("#modal_foto2").val(alibabas_edit.foto);
					$('#modal_harga').val(alibabas_edit.harga);
					// $('#modal_deskripsi').data("wysihtml5").editor.setValue(tokopedias_edit.deskripsi);
					CKEDITOR.instances.editor1.setData(alibabas_edit.deskripsi);
					$('#modal_berat').val(alibabas_edit.berat);
					$('#modal_stock').val(alibabas_edit.stock);
					$('#modal_categori1').val(alibabas_edit.cat1).trigger('change');
					// $('#modal_categori2').val(alibabas_edit.cat2).trigger('change');
					// $('#modal_categori3').val(alibabas_edit.cat3);
					$('#modal_ukuran').val(alibabas_edit.ukuran).change();
					$('#modal_warna').val(alibabas_edit.warna);
					var id_cate2 = alibabas_edit.cat2;
					var id_cate3 = alibabas_edit.cat3;
						// console.log(id_cat2);
						$.get('{{ URL::to("ajaxcat2/") }}/' + alibabas_edit.cat1, function(data){
							$('#modal_categori2').empty();
							$.each(data, function(index, id_cat2){
								var select="";
					...
					$.get('{{ URL::to("ajaxcat3/") }}/' + alibabas_edit.cat2, function(data){
